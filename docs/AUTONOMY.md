...
- پایش هر سه ساعت و بازسازی خودکار حداکثر سه مشکل با regression test در worktree جدا
...
این command task را با فاصلهٔ سه‌ساعته و نقطهٔ شروع 03:17 و shortcut داشبورد روی Desktop می‌سازد. هر اجرا می‌تواند سیگنال‌ها را تحلیل و حداکثر سه مشکل را با تست قرمز در worktree جدا اثبات کند، اما اجازهٔ تغییر کد محصول، ساخت branch یا PR ندارد. برای اصلاح باید انسان همان مورد را صریحاً تأیید کند. کامپیوتر باید به برق متصل باشد و wake timer در Windows فعال باشد.
