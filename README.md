...
ادمین ترب از مسیر `/admin` باز می‌شود. ورود فقط با `ADMIN_PASSWORD` است. مسیر درست این است که سیستم با سرچ تصویری/متنی، محصول متناظر ترب را پیدا کند و ادمین نتیجه درست را تایید کند؛ ورود دستی `base_product_rk` فقط fallback اضطراری است تا اتصال رسمی سرچ ترب کامل شود.
